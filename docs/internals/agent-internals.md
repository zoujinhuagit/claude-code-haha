---
title: 多 Agent 实现原理
nav_title: 多 Agent 实现
description: 多 Agent 的生成路径、工具池过滤、上下文传递与后台任务引擎。
order: 5
---
...
Open AI Ma Zai 的多 Agent 系统由以下核心模块组成：
...
| 工具 | 禁止原因 |
|------|----------|
| TaskOutput | 仅主 Agent 可读取任务输出 |
| ExitPlanMode | 仅主 Agent 可退出计划模式 |
| EnterPlanMode | 仅主 Agent 可进入计划模式 |
| AskUserQuestion | 子 Agent 不应直接问用户 |
| TaskStop | 仅主 Agent 可终止任务 |
...
Custom System Prompt       ← --system-prompt 参数
  ↓
Default System Prompt      ← Open AI Ma Zai 标准提示词
  ↓
...
| 文件 | 职责 |
|------|------|
| `src/utils/forkedAgent.ts` | 缓存安全参数，子 Agent 上下文 |
