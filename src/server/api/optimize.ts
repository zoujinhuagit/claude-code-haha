...
import Anthropic, { APIConnectionError, type ClientOptions } from '@anthropic-ai/sdk'
...
export async function handleOptimizeApi(req: Request): Promise<Response> {
  ...
  try {
    ...
    const hasAuth = Boolean(apiKey || authToken)
    const client = new Anthropic({
      baseURL,
      // 有认证时避免发送 dummy apiKey（auth_token 策略下 ANTHROPIC_API_KEY 为空）
      apiKey: hasAuth ? (apiKey || undefined) : 'optimize-dummy',
      ...(authToken ? { authToken } : {}),
      maxRetries: 1,
